...
func TestSyncReplicationControllerNeverDoesNothing(t *testing.T) {
	...
	manager.controllerStore.Store.Add(controllerSpec)
	newPodList(manager.podStore.Store, 2, api.PodFailed, controllerSpec)
...
func TestSyncReplicationControllerAlwaysCreates(t *testing.T) {
	...
	controller := newReplicationController(2, api.RestartPolicyAlways)
...
func TestPodFilteringAlways(t *testing.T) {
	// This rc is not needed by the test, only the newPodList to give the pods labels/a namespace.
	rc := newReplicationController(0, api.RestartPolicyAlways)
	...
	got := filterPods(podList.Items, api.RestartPolicyAlways)
...
func TestPodFilteringOnFailure(t *testing.T) {
	// This rc is not needed by the test, only the newPodList to give the pods labels/a namespace.
	rc := newReplicationController(0, api.RestartPolicyOnFailure)
	podList := newPodList(nil, 5, api.PodRunning, rc)
	podList.Items[0].Status.Phase = api.PodFailed
	podList.Items[1].Status.Phase = api.PodSucceeded
	expectedNames := util.NewStringSet()
	for _, pod := range podList.Items[1:] {
		expectedNames.Insert(pod.Name)
	}

	got := filterPods(podList.Items, api.RestartPolicyOnFailure)
	gotNames := util.NewStringSet()
	for _, pod := range got {
		gotNames.Insert(pod.Name)
	}
...
func TestPodFilteringNever(t *testing.T) {
	// This rc is not needed by the test, only the newPodList to give the pods labels/a namespace.
	rc := newReplicationController(0, api.RestartPolicyNever)
	podList := newPodList(nil, 5, api.PodRunning, rc)
	podList.Items[0].Status.Phase = api.PodFailed
	podList.Items[1].Status.Phase = api.PodSucceeded
	expectedNames := util.NewStringSet()
	...
	got := filterPods(podList.Items, api.RestartPolicyNever)
	gotNames := util.NewStringSet()
	for _, pod := range got {
		gotNames.Insert(pod.Name)
	}
